...
MISSION_DURATIONS  = [5, 10, 15, 20]  // the four rungs the board rolls (size at full price)
MISSION_CLOCK      = L1-10: 30/60/90/120 s by size (first errand always 30 s); L11+: size in minutes
MISSION_VIGOR_PER_MIN = 1             // cost = clock minutes, so L1-10 pay 0.5/1/1.5/2 and the
                                      // payout scales with the cost — income per vigor is level-flat
...
| `gem-strategies` | **drake-first pays ≤ 20% of ale-max's attribute power and buys ≥ 20% of the wall clock back per mission** at day 120 (the Drake sells time, not power — re-stated B1, see §10); **gacha-max no more than 18% behind** the better of the two, and ahead of both on pets, frames **and named legendaries** owned. Measured over a **16-seed** pool (widened M9); pity trips a pool mean > 8 times, never fewer than 4 on any seed |
...
|---|---|---|---|
| 2026-07-29 | **B2 vigor is priced off the clock: new `MISSION_VIGOR_PER_MIN` = 1**, `MISSION_CLOCK_BANDS` re-cut to whole half-minutes (30/60/90/120 s, was 30/50/70/90), and a mission's payout now scales with the vigor it costs instead of the rung it rolled. Save schema **v7 → v8**: an offer carries its `vigorCost` | B1 compressed the early clock and left the price at the mission's nominal size, so the board advertised "30s · 15 vigor" — half a minute of work charged at fifteen minutes' rates. The same split handed levels 1–10 a silent **10× discount on time**: a whole day's vigor burned through in eight minutes, at full income. Both are one bug. An early mission is now a SMALL mission — short clock, small price, small payout — so **income per vigor is identical at level 1 and level 60** and the compression cannot be a discount (asserted directly: "income per vigor is the same on both sides of the band"). The bands moved to half-minutes because they are prices now, and 50 s would be a cost of 0.83 vigor. Cost reads off the BAND clock, never the mount-shortened one — otherwise an Ember Drake would halve every price and double the day's income, which is precisely what §8.2's drake row exists to stop | all 8 anti-rush ceilings unmoved (day-1 **L12 ≤ 13**, day-30 L62 ≤ 62, day-90/180 unchanged); `gem-strategies` green. **Pacing did move**: an optimal day-1 now runs **239 missions** against ~25, because 250 vigor at 0.5–2 a mission is a great many small jobs. It is confined to day 1 — optimal play clears level 10 inside it, and every later day is back to ~20 missions. The lever if that is too many is the early vigor budget, not the clock |
| 2026-07-29 | **B2 the unlock ladder moves into `constants.ts`** — `PATROL_UNLOCK_LEVEL` 3, `ARENA_` 5, `WHEEL_` 5, `QUESTS_` 6, `ACHIEVEMENTS_` 6, `EXPEDITIONS_` 8, `SHOPS_` 10, `DUNGEONS_` 12, `FORGE_` 15, `HALL_OF_FAME_` 15; `PATROL_VIGOR_THRESHOLD` 5 relocated out of `engine/patrol.ts`. **No gate moved**, with one exception: the rail now advertises the Stable at **1**, which is where B1 put it | invariant 5, enforced after it bit. `ui/nav.ts` held its own literal copy of every unlock level, so when B1 moved `STABLE_UNLOCK_LEVEL` 10 → 1 the door opened and the sign on it did not — the rail went on printing "Stable · Lv 10" for a whole milestone, and the e2e spec that should have caught it was asserting the rail's private copy against itself. Values are now imported, never written, so a ladder change cannot be half-applied. GAME_DESIGN §18's table moved the Stable to the level-1 row in the same commit | no constant's **value** changed; all 8 anti-rush ceilings and `gem-strategies` green |
...
| 2026-07-29 | **§8.2 `gem-strategies` drake row re-stated from measurement**: was "drake-first within 12% of ale-max on power"; now **"pays ≤ 20% of ale-max's attribute power AND buys back ≥ 20% of the wall clock per mission"** | The old row only worked while the Drake was a one-off 60 gems — cheap enough that its cost hid inside the noise. As a fortnightly rental the real shape surfaced: **vigor meters the day, not the clock**, so a mount never earns a point of anything, it hands back hours. Measured over 16 seeds × 120 days: drake-first pays **14.6%** of attribute power and spends **4.41 min/mission against ale-max's 6.22** (ratio 0.709, per-seed 0.693–0.723) — **129 real-world hours** saved. Judging that on `powerScore` alone was judging a time-saver by a yardstick that cannot see time, and it duly called the Stable's headline animal a trap. The row is now two-sided, so if the mount ever stops working the clock assertion fails even though the power one would pass | drake cost 14.6% ≤ 20%; clock ratio 0.709 ≤ 0.8; gacha deficit 14.8% ≤ 18% |
| 2026-07-29 | **B1 simulator fidelity** (no constant changed): the sim now stops taking missions that would still be running at midnight, and reports `missionMinutes` / `missionsRun` | The day loop only ever checked vigor, so the model would run twenty 20-minute missions back to back however many hours that took — which is why it could see a mount's whole cost and none of its benefit. Same shape as the M7 set-piece bug: the simulator was quietly not playing the game. The midnight guard does not currently bite at optimal play (250 vigor of mixed missions finishes by early afternoon) but it is the real rule, and `missionMinutes` is what the re-stated §8.2 row is measured on | no ceiling moved |
| 2026-07-29 | **M9 changelog reconciliation** — no value changed. Every number this file publishes was read back out of `engine/constants.ts` and matched: toss 5/45, well unlock 18, stable 10, menagerie 35, shop-discount cap 0.5, dupe dust 2/3 and 40 treats, treat faucets 1/2, pet curve 1.5^1.1, well gear +5/+8, bundle 2.5/8/12 + 25% elixir, dungeon gems 1/3/5, patrol 8 h / 30 min. Backfills the one row M8 owed: `INSTALL_PROMPT_MIN_DAYS` 1 | invariant 5 says a constant may not exist here without a row, and M8 shipped one without. Backfilled rather than quietly grandfathered — the value is a UX gate (do not ask to install on somebody's first evening), not a balance lever, but the rule does not have an exemption for that | no sim movement; all §8.2 bounds unchanged |
| 2026-07-29 | **M9 uniques (§6.2) seeded**: `UNIQUE_DROP_SHARE` 0.35 — the share of legendary drops from the wheel jackpot and the Well's 1% row that become one of the eight NAMED legendaries instead of a generated one; each unique carries a `minLevel` gate (30–55) | invariant 5 — the one new number M9 adds. Uniques deliberately get **no faucet of their own**: they ride the pacing of the systems that already pay legendaries, so adding them cannot inflate the legendary supply. 0.35 lands all eight well past the 120-day horizon (measured 1.7 / 2.6 / 1.4 owned at day 120 for ale-max / gacha-max / drake-first) which is the intended "long tail's punchline" pace | `gem-strategies` green; all 8 anti-rush ceilings unmoved |
| 2026-07-29 | **M9 `gem-strategies` seed pool 8 → 16** (no bound changed) | the contract's headline number was not stable enough to gate a release on. Four-seed subsets of the same pool measure the gacha deficit anywhere from **11.3% to 22.2%**, and the two eight-seed halves sit 2 points apart — so a change that merely re-aligns the RNG streams (as M9's extra draws did) could swing it by more than the margin to the 18% ceiling, failing CI for a non-reason. Sixteen seeds halves that wobble for ~22s of CI. The 12%/18% bounds themselves are product statements and were **not** moved | deficit 15.55% ≤ 18% (was 14.1% on 8 seeds pre-M9); drake/ale spread 0.47% ≤ 12% |
| 2026-07-29 | **§8.2 `gem-strategies` pity bound re-stated**: per-seed `pityHits > 5` → **pool mean > 8 with a per-seed floor > 3** | the old bound asserted one seed's luck rather than the system's behaviour. Pity hits are high-variance — measured 5–17 across 20 seeds (mean 11.1, sd 3.0) — so a per-seed bound one standard deviation from the sample minimum breaks on any RNG realignment, which is exactly how it surfaced. The claim being made ("hundreds of tosses trip pity repeatedly") is a magnitude claim, and every other magnitude claim in this scenario is already a pool mean | mean 11.3, min 6 across the 16-seed pool |
...
| 2026-07-28 | **§8.2 `gem-strategies` re-stated** (M7): was "all three within 12%"; now **drake-first within 12% of ale-max**, and **gacha-max ≤ 18% behind** the better of the two while leading both on pets and frames | measured, not assumed. Over 8 seeds × 120 days: ale-max 4728 · drake-first 4833 (2.2% apart — the Ember Drake is never a mistake, which is the real trap to avoid) · gacha-max 4152. Three economy levers were tried against the gap and all bounced off (bundle gold 1.2→4.0, a 25% elixir, well gear +5 ilvl): gems→vigor→XP→**levels** compounds, and gear cannot match it. That is a genuine property of the economy, not a bug, so the contract now says what is true — all-in gacha **trades ~14% of attribute power for the collection** (more pets, more frames, sets completed; by day 270 it owns all 16 pets), which is a playstyle, not a trap. The 12% band is kept where it still bites: between the two strategies that both buy power. The measure was checked for bias against the strategy that buys *gear*: re-scored with armour, weapon damage and max HP included, the spread **widens to 25.9%**, because gacha's hero is ~11 levels behind and HP scales with level — so the attribute-only 14% is the conservative reading, not a flattering one | `gem-strategies` green; ale/drake 2.2% apart, gacha deficit 14.1% ≤ 18% |
